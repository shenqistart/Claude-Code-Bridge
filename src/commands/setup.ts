...
export async function setupCommand(provider?: string, apiKey?: string, options: SetupOptions = {}): Promise<void> {
  try {
    log.progress('开始配置 Claude Code...');
    ...
    log.error(`配置过程中发生错误: ${error instanceof Error ? error.message : String(error)}`);
    process.exit(1);
  }
}
...
async function configureProvider(provider?: string, apiKey?: string, options: SetupOptions = {}): Promise<void> {
