...
async function setupCommand(provider, apiKey, options = {}) {
    try {
        logger_1.log.progress('开始配置 Claude Code...');
        ...
        logger_1.log.error(`配置过程中发生错误: ${error instanceof Error ? error.message : String(error)}`);
        process.exit(1);
    }
}
